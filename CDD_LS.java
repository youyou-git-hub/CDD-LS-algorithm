...
public class CDD_LS {
    ...
    void initialization() throws IOException {
        filename = "/Users/qinhuigang/Desktop/双连通支配集/算例/v200_d5.dat";
        ...
        for (int i = 0; i < length; i++) {
            ...
            if (D_zpjplus.size() != 0) {
                ...
                for (Integer k : D_bzpjplus) {
                    if (V_degree[k] == 0 || disturbAraay[k] == 1) {
                        D_bzpjplus.remove(k);
                    }
                }
                int randomnumber1 = r.nextInt(D_bzpjplus.size() + 1);
                for (Integer k : D_bzpjplus) {
                    randomnumber1--;
                    if (randomnumber1 == 0) {
                        x1 = k;
